    ...
    <section class="mx-auto w-full max-w-5xl px-4" :class="props.vehicle || form.processing ? 'py-8' : 'min-h-[calc(100vh-4rem)] flex flex-col justify-center py-16'">
      <Reveal mode="up">
        <div :class="props.vehicle || form.processing ? '' : 'max-w-2xl mx-auto text-center'">
          <h1 class="font-bold tracking-tight" :class="props.vehicle || form.processing ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-4xl'">
            <span class="text-[#FFD700]">Registration</span> Lookup
          </h1>
          <p class="text-neutral-300" :class="props.vehicle || form.processing ? 'mt-1 text-sm' : 'mt-3 text-base'">
            Enter your vehicle registration for instant DVLA details including make, model, tax status, and MOT information.
          </p>

          <!-- Search Form -->
          <form @submit.prevent="submit" :class="props.vehicle || form.processing ? 'mt-4' : 'mt-8'">
            <div class="flex gap-3">
              <div class="flex-1">
                <input 
                  ...
                  maxlength="8"
                  class="w-full rounded-lg border border-neutral-800 bg-black/50 px-4 text-sm uppercase outline-none ring-2 ring-transparent placeholder:text-neutral-500 focus:border-[#FFD700]/50 focus:ring-[#FFD700]/20 transition-all"
                  :class="props.vehicle || form.processing ? 'py-3' : 'py-4'"
              ...
              <button 
                type="submit" 
                :disabled="form.processing"
                class="relative rounded-lg bg-[#FFD700] px-6 text-sm font-semibold text-black hover:brightness-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all overflow-hidden"
                :class="props.vehicle || form.processing ? 'py-3' : 'py-4'"
              >
      ...
      <Reveal v-if="props.vehicle && !form.processing" mode="up" :delay="120">
        <div class="mt-6 rounded-xl border border-[#FFD700]/30 bg-gradient-to-br from-neutral-900 to-neutral-950 p-4 shadow-lg animate-in fade-in duration-500">
          <!-- Header -->
          <div class="flex items-center justify-between border-b border-neutral-800 pb-3">
            <div>
              <h2 class="text-xl font-bold text-white">{{ props.vehicle.registration }}</h2>
              <p class="mt-0.5 text-sm text-neutral-400">{{ props.vehicle.make }} • {{ props.vehicle.year_of_manufacture }}</p>
            </div>
            <div class="rounded-full bg-[#FFD700]/20 p-2 animate-pulse">
              <CheckCircle class="h-5 w-5 text-[#FFD700]" />
            </div>
          </div>
          ...
          <div class="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            <!-- Make & Year -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Make & Year</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.make }}</p>
              <p class="text-xs text-neutral-400">{{ props.vehicle.year_of_manufacture }}</p>
            </div>

            <!-- Colour -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Colour</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.colour }}</p>
            </div>

            <!-- Fuel Type -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Fuel Type</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.fuel_type }}</p>
            </div>

            <!-- Engine Size -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Engine Size</p>
              <p class="mt-1 text-base font-semibold text-white">{{ formatEngineSize(props.vehicle.engine_capacity) }}</p>
            </div>

            <!-- Tax Status -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Tax Status</p>
              <p class="mt-1 text-base font-semibold" :class="props.vehicle.tax_status === 'Taxed' ? 'text-green-400' : 'text-red-400'">
                {{ props.vehicle.tax_status }}
              </p>
              <p v-if="props.vehicle.tax_due_date" class="text-xs text-neutral-400">Due: {{ props.vehicle.tax_due_date }}</p>
            </div>

            <!-- MOT Status -->
            <div class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">MOT Status</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.mot_status }}</p>
            </div>

            <!-- CO2 Emissions (if available) -->
            <div v-if="props.vehicle.co2_emissions" class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">CO₂ Emissions</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.co2_emissions }}g/km</p>
            </div>

            <!-- Euro Status (if available) -->
            <div v-if="props.vehicle.euro_status" class="rounded-lg border border-neutral-800 bg-neutral-900/50 p-3 transition-all hover:border-[#FFD700]/30 hover:bg-neutral-900/70">
              <p class="text-xs text-neutral-500 uppercase tracking-wide">Euro Status</p>
              <p class="mt-1 text-base font-semibold text-white">{{ props.vehicle.euro_status }}</p>
            </div>
          </div>
            ...
            <div class="flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <div class="rounded-lg bg-[#FFD700]/20 p-2">
                  <Phone class="h-5 w-5 text-[#FFD700]" />
                </div>
                <div>
                  <h3 class="text-base font-semibold text-white">Need tyres or service?</h3>
                  <p class="text-xs text-neutral-400">Call us for a quote or booking</p>
                </div>
              </div>
              <a 
                :href="`tel:${phone.replace(/\s+/g, '')}`" 
                class="inline-flex items-center gap-2 rounded-lg bg-[#FFD700] px-4 py-2 text-sm font-semibold text-black hover:brightness-95 transition-all whitespace-nowrap"
              >
                <Phone class="h-4 w-4" />
